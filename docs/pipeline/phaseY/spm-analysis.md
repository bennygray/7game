...
> **创建日期**: 2026-03-31
> **更新日期**: 2026-03-31（同步 V0.4.9 代码实际状态 + 环境适配性审查）
> **触发**: Phase X 前端 Review 23 项 + 后端 Review 15 项，且项目零自动化质量工具
> **目标**: 建立自动化 + 人工双层质量防线，前后端统一治理，杜绝同类问题再发
> **实际进展**: V0.4.9 已修复 21/37 项（前端 10 + 后端 11），剩余 16 项中 ~4 项为低优先级
...
> **重要**: V0.4.9 已修复大部分问题，以下为实际剩余统计。

| | 原始问题 | 已修复 | 剩余 |
|---|:---:|:---:|:---:|
| 前端 | 23 | 10 (P0×4 + P1×4 + P2×1 + P3×1) | **13** |
| 后端 | 15 (14 in Y) | 11 (P0×2 + P1×4 + P2×5) | **3** (P3 低优先级) |
| **合计** | 37 | **21** | **16** |
...
> **环境适配性警告**: 以下方案原为 Claude Code CLI 设计，在其他 AI 工具环境中不可用。

1. ~~**PostToolUse Hook**: 每次 Edit/Write 自动 lint 当前文件~~ — ❌ 仅 Claude Code CLI 可用
2. ~~**ESLint MCP Server**（`@eslint/mcp`）: 官方 MCP，可在 Claude Code 中直接调用~~ — ❌ 仅 Claude Code CLI 可用
3. **SGE Skill 增强**: Gate 3 追加 Code Quality Checklist — ✅ 通用

**替代方案**（适用于任意 AI 工具环境）:
- `npm run lint` 手动/脚本触发
- IDE 层面 ESLint on-save（VSCode settings.json）
- `.agents/workflows/` 中创建 lint workflow
...
Step 1: ESLint 基础配置（Y-1 + Y-2 + Y-9）
  ...
  → 注意: eslint-plugin-import 用 import-x 替代（flat config 兼容）
  → 注意: eslint-plugin-clean-timer 可能不支持 ESLint 9，建议跳过
  → 首次 lint 观察错误数量，决定 error/warn 级别

Step 2: 前端遗留修复（Y-5 剩余项）
  → P0+P1 已全部修复（V0.4.9），跳过
  → P1-03: status 格式化抽取到 mud-formatter
  → P2-03: 弟子未找到消息格式统一
  → P2-05: escapeHtml 迁移到 shared/utils
  → P2-08: 命令别名统一到映射表
  → lint 零 error 通过

Step 3: 后端存量修复 — ✅ 已完成（V0.4.9）
  → 11/14 项已修复，仅剩 P3 低优先级、按需处理

Step 4: SGE Skill 增强（Y-4 + Y-10）
...
| ~~PostToolUse Hook 拖慢 Claude Code~~ | ~~编辑卡顿~~ | ❌ 当前环境不适用（非 Claude Code CLI） |
| `eslint-plugin-import` flat config 不兼容 | lint 报错 | 用 `eslint-plugin-import-x` 替代 |
| `eslint-plugin-clean-timer` 可能不支持 ESLint 9 | 插件不可用 | 跳过，用 SGE Checklist 人工覆盖 |

...
|-----|-----------------|----------------|----------------|
| 1 | 新游戏创建 | `createDefaultLiteGameState()` 调用 | 每个弟子拥有 `moral: { goodEvil, lawChaos }` 字段，值在 [-30, +30] 范围内 |
| 2 | 新游戏创建 | 弟子生成 | 每个弟子拥有 1~2 个先天特性 `traits: DiscipleTrait[]`，每个 trait 的 `defId` 存在于 `TRAIT_REGISTRY` 中 |
| 3 | `TRAIT_REGISTRY` 已定义 | 查看注册表 | 包含 ≥10 个 TraitDef，覆盖 positive/negative/neutral 三极，人性核心主题 |
| 4 | 新游戏创建 | 关系边生成 | `RelationshipEdge` 包含 `affinity`（[-20,+20]，受道德相似度影响）、`tags: []`、`lastInteraction` |
...
| AC# | Given (前置条件) | When (触发动作) | Then (预期结果) |
|-----|-----------------|----------------|----------------|
| 1 | 事件类型=`alchemy-success`，反应者角色=`observer` | `buildCandidatePool(event, subject)` | 返回 4 个候选情绪（如 `['joy', 'envy', 'admiration', 'neutral']`），非空 |
| 2 | 事件类型=`betrayal`，反应者角色=`victim` | `buildCandidatePool(event, subject)` | 返回 `['anger', 'sadness', 'contempt', 'fear']` 等负面情绪候选 |
| 3 | AI 输出 `delta=+10`，反应者角色=`victim`，事件类型=`theft` | `correctDeltaDirection(delta, role, eventType)` | 返回 `-10`（受害者在负面事件中 delta 强制为负） |
| 4 | AI 输出 `delta=+8`，反应者角色=`beneficiary`，事件类型=`sacrifice` | `correctDeltaDirection(delta, role, eventType)` | 返回 `+8`（受益者在正面事件中保持正向） |
| 5 | AI 输出 `delta=+15`（超出范围） | `clampDelta(delta)` | 返回 `+5`（R-E11 夹值 [-5, +5]） |
| 6 | 反应者角色=`observer` | `correctDeltaDirection(delta, role, eventType)` | 返回原始 delta（观察者不干预方向） |
...
| AC# | Given (前置条件) | When (触发动作) | Then (预期结果) |
|-----|-----------------|----------------|----------------|
| 1 | 弟子 A 炼丹成功，AI 后端可用 | SoulEvaluator 处理事件 | 为相关弟子生成 `SoulEvaluationResult`（emotion + intensity + deltas + innerThought），JSON 格式正确 |
| 2 | 弟子 A 炼丹成功，AI 后端**不**可用 | SoulEvaluator fallback | 使用规则引擎从候选池随机选择情绪，delta 使用确定性公式计算，innerThought 使用模板生成 |
| 3 | AI 返回结果 | 后处理执行 | delta 方向修正 + 夹值到 [-5, +5] + 写入 GameState 关系边 |
| 4 | 关系边 affinity 变化后 | 标签检查 | affinity ≥60 自动添加 `friend` 标签；≤-60 自动添加 `rival` 标签 |
| 5 | AI 评估完成 | MUD 日志输出 | 格式：`[灵魂] 张清风感到 愤怒(3)：「此乃天下之大不义！」`，颜色区分于普通行为日志 |
| 6 | 关系衰减时间到达 | 每 5 分钟 tick | 所有关系边 affinity 乘以 0.98 向 0 衰减 |
...
| 3 | 运行 5 分钟后 | 检查关系边 | 至少 1 对弟子的 affinity 从初始值发生变化（被 AI 评估或规则引擎修改） |
| 4 | 回归测试 | `npx tsx scripts/regression-all.ts` | 全部现有测试通过 + 新增 Phase E 专项测试通过 |
| 5 | Phase E 验证脚本 | `npx tsx scripts/verify-phaseE.ts` | 10 组数值验证通过：道德初始范围、特性分配、关系衰减、delta 夹值、标签触发 |
